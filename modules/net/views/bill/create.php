    ...
    <div class="box-body">
        <div class="row"><div id="w1"><div class="grid-view col-xs-12 table-responsive" id="w2">
                ...
                <tbody><?php foreach($arrBillDetail as $i=>$item): ?>
                    <?php
                        ...
                        $dataset = ['item_no'=>$item->item_no, 'item_name'=>$item->item_name, 'price'=>$item->price,
                            'pay_date'=>$item->pay_date, 'description'=>$item->description, 'delete_flag'=>$item->delete_flag];
                        $datasetStr = json_encode($dataset, JSON_NUMERIC_CHECK);

                        $arrBtns = [];
                        $arrBtns[] = StringUtils::format("<li><a href='javascript:void(0)' onclick='editBillItem(this);' data-maps='{0}'>{1}</a></li>", [$datasetStr, $lblEdit]);
                        $arrBtns[] = StringUtils::format('<li><a href="javascript:void(0)" onclick="deleteBillItem(this);" data-message-delete-confirm="{0}" data-item-no="{1}">{2}</a></li>', [Yii::t('message', 'Do you want to delete this item ?'), $item->item_no, $lblDetele]);

                        $htmlAction = '<div class="btn-group">';
                        $htmlAction .= StringUtils::format("<a href='javascript:void(0)' onclick='editBillItem(this);' data-maps='{0}' class='btn btn-{1}'>{2}</a>", [$datasetStr, $btnClass, $lblEdit]);
            ...
            <div class="form-group">
                ...
                <?= Html::submitButton(Yii::t('button', 'Save'), ['class'=>'btn btn-info', 'name'=>MasterValueUtils::SM_MODE_NAME, 'value'=>MasterValueUtils::SM_MODE_INPUT]); ?>
            </div>
        </div></div></div>
    </div>
